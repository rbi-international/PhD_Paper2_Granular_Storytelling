...
SAMPLES_PER_EMOTION = 20  # 160 Total Stories
...
def load_instruction_model():
    print(f"Loading Instruction-Tuned Model from {ADAPTER_PATH}...")
...
def main():
    student_model, student_tokenizer = load_instruction_model()
    ...
    print(f"\n--- Starting INSTRUCTION Evaluation ({SAMPLES_PER_EMOTION} samples/emotion) ---")

    for target_emotion in EMOTIONS:
        # Handling the merged class logic for evaluation fairness
        expected_label = "Anger" if target_emotion == "Disgust" else target_emotion
        
        print(f"Testing: Emotion: {target_emotion} | Story: ...")
        
        for _ in range(SAMPLES_PER_EMOTION):
            # THE KEY CHANGE: Prompting with the instruction format
            input_text = f"Emotion: {target_emotion} | Story: {PROMPT_STORY_START}"
            ...
            with torch.no_grad():
                outputs = student_model.generate(
                    **inputs, 
                    max_new_tokens=80, 
                    do_sample=True, 
                    temperature=0.7,   # Slightly lower temp for coherence
                    top_p=0.9, 
            ...
            # Post-Processing: Extract just the story part
            # We look for "Story: " and take everything after it
            try:
                # Split by the separator we used in training
                generated_story = full_text.split("| Story:")[1].strip()
            except IndexError:
                # Fallback if model messes up the separator (rare with loss 1.17)
    ...
    }).to_csv("evaluation_results_instruction.csv", index=False)
    print("Saved detailed results to 'evaluation_results_instruction.csv'")
